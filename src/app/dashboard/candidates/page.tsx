...
export default function AllCandidatesPage() {
  ...
  const filteredCandidates = candidates.filter(candidate => {
    ...
    const matchesJob = 
      selectedJob === 'all' || candidate.job_id === selectedJob
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center font-semibold text-primary border border-primary/20 shrink-0">
                            {candidate.full_name ? candidate.full_name.charAt(0).toUpperCase() : (candidate.email || '?').charAt(0).toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <div className="font-medium text-foreground truncate">{candidate.full_name || 'No Name'}</div>
